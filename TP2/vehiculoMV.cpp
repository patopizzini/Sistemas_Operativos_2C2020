...
int main()
{
	...
	cout << endl;
	cout << "TP2 - Ruta - Vehiculo MV" << endl;
	...
	if (p_contador->n_VM > 0) {
		...
		//Se incrementa el contador de esperas en la cabecera del monte
		p_contador->c_MV = p_contador->c_MV + 1;
		
		//Mensaje de espera
		cout << endl;
		cout << "Hay vehículos circulando en la dirección VM." << endl;
		cout << "Esperando..." << endl;
	...
	if (p_contador->n_MV == 0) {
		...
		for (int i=0; i<p_contador->c_VM; i++) {
	...
	cout << "VehiculoMV terminado." << endl;
